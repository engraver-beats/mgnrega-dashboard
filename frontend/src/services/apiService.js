...
class APIService {
  ...
  // ✅ Returns true / false (required in districtService)
  async isBackendAvailable() {
    try {
      const health = await this.checkHealth();
  ...
  // ✅ Get district map data (ID + Optional Year)
  async getDistrictData(districtId, year = null) {
    if (!districtId) throw new Error('District ID required');

    let url = `/districts/${districtId}`;
  ...
  async searchDistricts(query) {
    return await this.request(`/districts/search?q=${encodeURIComponent(query)}`);
  }

  // ✅ Get districts by state name
  async getDistrictsByState(stateName) {
    return await this.request(`/states/${encodeURIComponent(stateName)}/districts`);
  }
  ...
  // ✅ Auto-detect user location via backend
  async detectUserLocation() {
    return await this.request('/location/detect');
  }

  // ✅ Backend load financial years
  async getFinancialYears() {
    return await this.request('/financial-years');
  }

  // ✅ Trigger backend refresh
  async refreshData() {
    return await this.request('/refresh', { method: 'POST' });
  }
}
