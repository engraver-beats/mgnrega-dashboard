...
export async function getAllDistricts() {
  try {
    ...
    console.error('❌ Failed to fetch all districts:', err.message);
    return [];
  }
}
...
export async function searchDistricts(query = "") {
  try {
    ...
    console.error("❌ searchDistricts failed:", err);
    return [];
  }
}
...
export async function getDistrictById(id) {
  try {
    ...
    console.error(`❌ District fetch failed for ${id}`, err.message);
    return null;
  }
}
...
export async function getDistrictData(id) {
  ...
  try {
    ...
    console.error("❌ districtService: getDistrictData failed", err);
    return null;
  }
}

// ✅ Detect user's location → find nearest district
